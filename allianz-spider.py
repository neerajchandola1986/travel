...
def lambda_handler(event, context):
    ...
    loc = event["queryStringParameters"].get("loc", False)
    ...
    if(url):
        ...
        if(travel_cost):
            driver.execute_script('$("#sq_viaggi_importo_assicurato").val("%s")' %travel_cost )
        ...
        if(coupen):
            driver.execute_script("$('#codice_sconto').val('%s');" %coupen )
        ...
        info = {}
        info["price"] = str(driver.execute_script("return $('.pp_thead_titles .offer_price .price')[0].firstChild.data;"))
        if(coupen):
            info["d_price"] = str(driver.execute_script("return $('.pp_thead_titles .offer_price .price')[1].firstChild.data;"))
        info["bag_price"] = str(driver.execute_script(" return $('#bagaglio .destinazione_M,.day day_1_4').find('b').text(); "))
        info["bag_price"] = info["bag_price"].replace("€", "").strip()
        print(page_data)
    driver.close()
    return {
    "statusCode": 200,
